...
class GmailAutomation:
    ...
    # import resume
    def importPdfForUpload(self) -> PdfReader:
        return PdfReader('TR_AdemHilmiBozkurt.pdf')
    ...
    def signIn(self, driver: webdriver): 
        ...
        time.sleep(2)
        email = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.XPATH, "//input[@type='email']")))
        email.clear()
    ...
    def sendMessage(self, driver: webdriver, receiver: str, subject: str, message: str, pdfForUpload: PdfReader) -> None:
        ...
        # upload a resume
        uploadAFile = WebDriverWait(driver, 30).until(EC.element_to_be_clickable((By.XPATH, "//input[@type='file']")))[1]
        uploadAFile.clear()
        uploadAFile.send_keys(pdfForUpload)
        time.sleep(3)
        
        # send mail
        send = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, "//div[contains(@aria-label,'Gönder')]|//div[contains(@aria-label,'Send')]")))[1].click() 
        time.sleep(5)
        
        
    def sendtoMultipleRecepients(self, driver: webdriver, contacts: [str], subject: str, message: str, pdfForUpload: PdfReader) -> None:    
        for contact in contacts:        
            ...
            # take definition from user input
            messageBox = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.XPATH, "//div[@role='textbox']/br")))
            subjectBox.clear()
            subjectBox.send_keys(message)
            time.sleep(3)
            
            # upload a resume
            uploadAFile = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.XPATH, "//input[@type='file']")))
            uploadAFile.clear()
            uploadAFile.send_keys(pdfForUpload)
            time.sleep(3)
